...
CODE_SAMPLE
    ...
    private function shouldSkipString(\PhpParser\Node\Scalar\String_ $string) : bool
    {
        ...
        $matches = \RectorPrefix20211231\Nette\Utils\Strings::match($value, '#(?<' . self::VALUE . '>[\\w\\-\\/\\_]+)#');
    ...
    private function createConstName(string $value) : string
    {
        ...
        $value = \RectorPrefix20211231\Nette\Utils\Strings::replace($value, self::SLASH_AND_DASH_REGEX, self::UNDERSCORE);
        // find beginning numbers
        $beginningNumbers = '';
        $matches = \RectorPrefix20211231\Nette\Utils\Strings::match($value, '#(?<' . self::NUMBERS . '>[0-9]*)(?<' . self::VALUE . '>.*)#');
        ...
        return \strtoupper(\RectorPrefix20211231\Nette\Utils\Strings::replace($value, '#_+#', self::UNDERSCORE));
    }
