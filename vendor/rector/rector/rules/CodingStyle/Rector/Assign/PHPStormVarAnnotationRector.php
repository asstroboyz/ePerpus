...
CODE_SAMPLE
    ...
    private function createDocComment(\PhpParser\Node $node) : \PhpParser\Comment\Doc
    {
        ...
        if (\strncmp($docContent, '/* ', \strlen('/* ')) === 0) {
            $docContent = \RectorPrefix20211231\Nette\Utils\Strings::replace($docContent, self::SINGLE_ASTERISK_COMMENT_START_REGEX, '/** ');
        }
        // $value is first, instead of type is first
        if (\Rector\Core\Util\StringUtils::isMatch($docContent, self::VAR_ANNOTATION_REGEX)) {
            $docContent = \RectorPrefix20211231\Nette\Utils\Strings::replace($docContent, self::VARIABLE_NAME_AND_TYPE_MATCH_REGEX, '$3$2$1');
        }
