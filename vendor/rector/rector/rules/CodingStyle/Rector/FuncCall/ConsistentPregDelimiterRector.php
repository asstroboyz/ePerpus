...
, [self::DELIMITER => '#'])]);
    ...
    {
        $matchInnerRegex = \RectorPrefix20211231\Nette\Utils\Strings::match($string, self::INNER_REGEX);
        $matchInnerUnionRegex = \RectorPrefix20211231\Nette\Utils\Strings::match($string, self::INNER_UNICODE_REGEX);
    ...
    private function refactorArgument(\PhpParser\Node\Arg $arg) : void
    {
        ...
        $string->value = \RectorPrefix20211231\Nette\Utils\Strings::replace($string->value, self::INNER_REGEX, function (array $match) use(&$string) : string {
