...
use RectorPrefix20211231\Controller;
use PhpParser\Node;
...
, [self::TYPES => ['ValueObjectInterfaceClassName'], self::SUFFIXES => ['Search'], self::ENABLE_VALUE_OBJECT_GUESSING => \true])]);
    ...
    public function configure(array $configuration) : void
    {
        ...
        \RectorPrefix20211231\Webmozart\Assert\Assert::isArray($types);
        \RectorPrefix20211231\Webmozart\Assert\Assert::allString($types);
        $this->types = $types;
        $suffixes = $configuration[self::SUFFIXES] ?? [];
        \RectorPrefix20211231\Webmozart\Assert\Assert::isArray($suffixes);
        \RectorPrefix20211231\Webmozart\Assert\Assert::allString($suffixes);
        $this->suffixes = $suffixes;
        $enableValueObjectGuessing = $configuration[self::ENABLE_VALUE_OBJECT_GUESSING] ?? \false;
        \RectorPrefix20211231\Webmozart\Assert\Assert::boolean($enableValueObjectGuessing);
