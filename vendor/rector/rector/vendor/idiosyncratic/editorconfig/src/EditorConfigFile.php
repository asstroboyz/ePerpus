...
final class EditorConfigFile
{
    ...
    public function __construct(string $path, ?\RectorPrefix20211231\Idiosyncratic\EditorConfig\Declaration\Factory $declarationFactory = null)
    {
        $this->declarationFactory = $declarationFactory ?? new \RectorPrefix20211231\Idiosyncratic\EditorConfig\Declaration\Factory();
    ...
    private function parse(string $content) : void
    {
        ...
        foreach ($parsedContent as $glob => $declarations) {
            ...
            $this->sections[] = new \RectorPrefix20211231\Idiosyncratic\EditorConfig\Section($this->getGlobPrefix($glob), $glob, $declarations, $this->declarationFactory);
        }
    }
    private function setIsRoot(string $isRoot) : void
    {
        if (\in_array($isRoot, ['true', 'false']) === \false) {
            throw new \RectorPrefix20211231\Idiosyncratic\EditorConfig\Exception\InvalidValue('root', $isRoot);
        }
