...
final class Reflection
{
    ...
    {
        $type = \RectorPrefix20211231\Nette\Utils\Type::fromReflection($func);
    ...
    {
        $type = \RectorPrefix20211231\Nette\Utils\Type::fromReflection($param);
        return $type ? $type->getNames() : [];
    }
    /**
    ...
    {
        $type = \RectorPrefix20211231\Nette\Utils\Type::fromReflection($prop);
        return $type ? $type->getNames() : [];
    }
    /**
    ...
    private static function getType($reflection, ?\ReflectionType $type) : ?string
    {
        if ($type === null) {
            ...
            return \RectorPrefix20211231\Nette\Utils\Type::resolve($type->getName(), $reflection);
        } elseif ($type instanceof \ReflectionUnionType || $type instanceof \RectorPrefix20211231\ReflectionIntersectionType) {
            throw new \RectorPrefix20211231\Nette\InvalidStateException('The ' . self::toString($reflection) . ' is not expected to have a union or intersection type.');
        } else {
            throw new \RectorPrefix20211231\Nette\InvalidStateException('Unexpected type of ' . self::toString($reflection));
        }
    }
    /**
    ...
    public static function getParameterDefaultValue(\ReflectionParameter $param)
    {
        if ($param->isDefaultValueConstant()) {
            ...
            $pair = \explode('::', $const);
            if (isset($pair[1])) {
                $pair[0] = \RectorPrefix20211231\Nette\Utils\Type::resolve($pair[0], $param);
                try {
    ...
    public static function toString(\Reflector $ref) : string
    {
        if ($ref instanceof \ReflectionClass) {
            ...
            throw new \RectorPrefix20211231\Nette\InvalidArgumentException();
        }
    }
    /**
    ...
    public static function expandClassName(string $name, \ReflectionClass $context) : string
    {
        ...
        if (empty($name)) {
            throw new \RectorPrefix20211231\Nette\InvalidArgumentException('Class name must not be empty.');
    ...
    public static function getUseStatements(\ReflectionClass $class) : array
    {
        if ($class->isAnonymous()) {
            throw new \RectorPrefix20211231\Nette\NotImplementedException('Anonymous classes are not supported.');
        }
        static $cache = [];
