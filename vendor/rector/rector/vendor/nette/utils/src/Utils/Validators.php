...
class Validators
{
    ...
    protected static $validators = [
        ...
        'list' => [\RectorPrefix20211231\Nette\Utils\Arrays::class, 'isList'],
    ...
    /** @var array<string,callable> */
    protected static $counters = ['string' => 'strlen', 'unicode' => [\RectorPrefix20211231\Nette\Utils\Strings::class, 'length'], 'array' => 'count', 'list' => 'count', 'alnum' => 'strlen', 'alpha' => 'strlen', 'digit' => 'strlen', 'lower' => 'strlen', 'space' => 'strlen', 'upper' => 'strlen', 'xdigit' => 'strlen'];
    /**
    ...
    public static function assert($value, string $expected, string $label = 'variable') : void
    {
        if (!static::is($value, $expected)) {
            ...
            throw new \RectorPrefix20211231\Nette\Utils\AssertionException("The {$label} expects to be {$expected}, {$type} given.");
        }
    }
    /**
    ...
    public static function assertField(array $array, $key, string $expected = null, string $label = "item '%' in array") : void
    {
        if (!\array_key_exists($key, $array)) {
            throw new \RectorPrefix20211231\Nette\Utils\AssertionException('Missing ' . \str_replace('%', $key, $label) . '.');
        } elseif ($expected) {
    ...
    public static function is($value, string $expected) : bool
    {
        foreach (\explode('|', $expected) as $item) {
            ...
            if (isset(static::$validators[$type])) {
                ...
                if (\RectorPrefix20211231\Nette\Utils\Strings::match($value, '|^' . ($item[1] ?? '') . '$|D')) {
                    return \true;
                }
                continue;
    ...
    {
        return \RectorPrefix20211231\Nette\Utils\Arrays::isList($value);
    }
    /**
