...
class Process
{
    ...
    public static function escape($arg, $meta = \true, $module = \false)
    {
        ...
        $arg = \RectorPrefix20211231\Composer\Pcre\Preg::replace('/(\\\\*)"/', '$1$1\\"', $arg, -1, $dquotes);
        if ($meta) {
            $meta = $dquotes || \RectorPrefix20211231\Composer\Pcre\Preg::isMatch('/%[^%]+%/', $arg);
            if (!$meta) {
        ...
        if ($quote) {
            $arg = '"' . \RectorPrefix20211231\Composer\Pcre\Preg::replace('/(\\\\*)$/', '$1$1', $arg) . '"';
        }
        if ($meta) {
            $arg = \RectorPrefix20211231\Composer\Pcre\Preg::replace('/(["^&|<>()%])/', '^$1', $arg);
        }
        return $arg;
    }
    /**
