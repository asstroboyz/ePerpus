...
final class StaticFixtureFinder
{
    ...
    private static function yieldFileInfos(array $fileInfos) : \Iterator
    {
        foreach ($fileInfos as $fileInfo) {
            try {
                ...
                (yield [$smartFileInfo]);
            } catch (\RectorPrefix20211231\Symplify\SmartFileSystem\Exception\FileNotFoundException $exception) {
            }
        }
    }
    /**
    ...
    {
        $finder = \RectorPrefix20211231\Symfony\Component\Finder\Finder::create()->in($directory)->files()->name($suffix);
    ...
    {
        $finder = \RectorPrefix20211231\Symfony\Component\Finder\Finder::create()->in($directory)->files()->notName($suffix);
        /** @var SplFileInfo[] $fileInfos */
        $fileInfos = \iterator_to_array($finder->getIterator());
        $relativeFilePaths = [];
        foreach ($fileInfos as $fileInfo) {
            $relativeFilePaths[] = \RectorPrefix20211231\Nette\Utils\Strings::substring($fileInfo->getRealPath(), \strlen(\getcwd()) + 1);
        }
        if ($relativeFilePaths === []) {
            return;
        }
        throw new \RectorPrefix20211231\Symplify\SymplifyKernel\Exception\ShouldNotHappenException(\sprintf('Files "%s" have invalid suffix, use "%s" suffix instead', \implode('", ', $relativeFilePaths), $suffix));
    }
}
