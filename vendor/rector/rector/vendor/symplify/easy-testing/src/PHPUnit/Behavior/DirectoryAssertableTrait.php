...
namespace RectorPrefix20211231\Symplify\EasyTesting\PHPUnit\Behavior;

use RectorPrefix20211231\Symfony\Component\Finder\Finder;
use RectorPrefix20211231\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair;
...
trait DirectoryAssertableTrait
{
    ...
    {
        $firstDirectoryFinder = new \RectorPrefix20211231\Symfony\Component\Finder\Finder();
        $firstDirectoryFinder->files()->in($directory);
        $finderSanitizer = new \RectorPrefix20211231\Symplify\SmartFileSystem\Finder\FinderSanitizer();
    ...
    private function groupFileInfosByRelativeFilePath(array $expectedFileInfos, string $expectedDirectory, array $outputFileInfos, string $outputDirectory) : array
    {
        ...
        foreach ($expectedFileInfos as $expectedFileInfo) {
            ...
            $fileInfosByRelativeFilePath[$relativeFilePath] = new \RectorPrefix20211231\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair($expectedFileInfo, $outputFileInfo);
        }
