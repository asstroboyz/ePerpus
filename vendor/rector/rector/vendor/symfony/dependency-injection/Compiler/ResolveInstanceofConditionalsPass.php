 ...
 */
class ResolveInstanceofConditionalsPass implements \RectorPrefix20211231\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    /**
     * {@inheritdoc}
     */
    public function process(\RectorPrefix20211231\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        foreach ($container->getAutoconfiguredInstanceof() as $interface => $definition) {
            if ($definition->getArguments()) {
                throw new \RectorPrefix20211231\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Autoconfigured instanceof for type "%s" defines arguments but these are not supported and should be removed.', $interface));
            }
        }
        $tagsToKeep = [];
    ...
    private function processDefinition(\RectorPrefix20211231\Symfony\Component\DependencyInjection\ContainerBuilder $container, string $id, \RectorPrefix20211231\Symfony\Component\DependencyInjection\Definition $definition, array $tagsToKeep) : \RectorPrefix20211231\Symfony\Component\DependencyInjection\Definition
    {
        ...
        $parent = $definition instanceof \RectorPrefix20211231\Symfony\Component\DependencyInjection\ChildDefinition ? $definition->getParent() : null;
        ...
        if ($parent) {
            ...
            if (\RectorPrefix20211231\Symfony\Component\DependencyInjection\Definition::class === \get_class($abstract)) {
    ...
    private function mergeConditionals(array $autoconfiguredInstanceof, array $instanceofConditionals, \RectorPrefix20211231\Symfony\Component\DependencyInjection\ContainerBuilder $container) : array
    {
        ...
        foreach ($instanceofConditionals as $interface => $instanceofDef) {
            ...
            if (!$container->getReflectionClass($interface)) {
                throw new \RectorPrefix20211231\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('"%s" is set as an "instanceof" conditional, but it does not exist.', $interface));
            }
