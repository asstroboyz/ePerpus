 ...
 */
class ArrayNode extends \RectorPrefix20211231\Symfony\Component\Config\Definition\BaseNode implements \RectorPrefix20211231\Symfony\Component\Config\Definition\PrototypeNodeInterface
{
        ...
        foreach ($this->children as $name => $child) {
            if (!\array_key_exists($name, $value)) {
                if ($child->isRequired()) {
                    ...
                    $ex = new \RectorPrefix20211231\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($message);
    ...
    protected function validateType($value)
    {
        if (!\is_array($value) && (!$this->allowFalse || \false !== $value)) {
            $ex = new \RectorPrefix20211231\Symfony\Component\Config\Definition\Exception\InvalidTypeException(\sprintf('Invalid type for path "%s". Expected "array", but got "%s"', $this->getPath(), \get_debug_type($value)));
    ...
    protected function normalizeValue($value)
    {
        ...
        if (\count($value) && !$this->ignoreExtraKeys) {
            ...
            $ex = new \RectorPrefix20211231\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($msg);
            $ex->setPath($this->getPath());
            throw $ex;
        }
        return $normalized;
    ...
    protected function remapXml(array $value)
    {
        foreach ($this->xmlRemappings as [$singular, $plural]) {
            ...
            $value[$plural] = \RectorPrefix20211231\Symfony\Component\Config\Definition\Processor::normalizeConfig($value, $singular, $plural);
