...
final class ServiceTypeResolver
{
     ...
     * @return string|null
     */
    public function resolve(\RectorPrefix20211231\Nette\Neon\Node $serviceNode)
    {
        if (!$serviceNode instanceof \RectorPrefix20211231\Nette\Neon\Node\ArrayItemNode) {
            return null;
        }
        if (!$serviceNode->value instanceof \RectorPrefix20211231\Nette\Neon\Node\ArrayNode) {
            return null;
        }
        foreach ($serviceNode->value->items as $serviceConfigurationItem) {
            ...
            if ($serviceConfigurationItem->key->toString() === self::FACTORY_KEYWORD) {
                if ($serviceConfigurationItem->value instanceof \RectorPrefix20211231\Nette\Neon\Node\EntityNode) {
