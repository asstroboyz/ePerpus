...
final class TemplateFileSystem
{
    ...
    public function resolveDestination(\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo, array $templateVariables, \Rector\RectorGenerator\ValueObject\RectorRecipe $rectorRecipe, string $targetDirectory) : string
    {
        ...
        if (!$rectorRecipe->isRectorRepository()) {
            ...
            $destination = \RectorPrefix20211231\Nette\Utils\Strings::replace($destination, self::PACKAGE_RULES_PATH_REGEX, 'utils/rector/src');
            $destination = \RectorPrefix20211231\Nette\Utils\Strings::replace($destination, self::PACKAGE_RULES_TESTS_PATH_REGEX, 'utils/rector/tests');
        }
        // remove _Configured|_Extra prefix
        $destination = $this->templateFactory->create($destination, $templateVariables);
        $destination = \RectorPrefix20211231\Nette\Utils\Strings::replace($destination, self::CONFIGURED_OR_EXTRA_REGEX, '');
        // remove ".inc" protection from PHPUnit if not a test case
        if ($this->isNonFixtureFileWithIncSuffix($destination)) {
            $destination = \RectorPrefix20211231\Nette\Utils\Strings::before($destination, '.inc');
        }
    ...
    {
        if (\RectorPrefix20211231\Nette\Utils\Strings::match($filePath, self::FIXTURE_SHORT_REGEX) !== null) {
            return \false;
        }
