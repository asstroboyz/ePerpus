...
class Helpers
{
    ...
    {
        $file = \strtr($origFile = $file, \RectorPrefix20211231\Tracy\Debugger::$editorMapping);
    ...
    {
        if (\RectorPrefix20211231\Tracy\Debugger::$editor && $file && ($action === 'create' || \is_file($file))) {
            $file = \strtr($file, '/', \DIRECTORY_SEPARATOR);
            $file = \strtr($file, \RectorPrefix20211231\Tracy\Debugger::$editorMapping);
            $search = \str_replace("\n", \PHP_EOL, $search);
            $replace = \str_replace("\n", \PHP_EOL, $replace);
            return \strtr(\RectorPrefix20211231\Tracy\Debugger::$editor, ['%action' => $action, '%file' => \rawurlencode($file), '%line' => $line ?: 1, '%search' => \rawurlencode($search), '%replace' => \rawurlencode($replace)]);
        }
        return null;
    }
    ...
    public static function improveException(\Throwable $e) : void
    {
        ...
        if (!$e instanceof \Error && !$e instanceof \ErrorException || $e instanceof \RectorPrefix20211231\Nette\MemberAccessException || \strpos($e->getMessage(), 'did you mean')) {
            // do nothing
        ...
        if (isset($hint)) {
            $loc = \RectorPrefix20211231\Tracy\Debugger::mapSource($e->getFile(), $e->getLine()) ?? ['file' => $e->getFile(), 'line' => $e->getLine()];
