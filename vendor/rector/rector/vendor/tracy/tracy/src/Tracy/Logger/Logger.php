 ...
 * Logger.
 */
class Logger implements \RectorPrefix20211231\Tracy\ILogger
{
     ...
     */
    public function __construct(?string $directory, $email = null, ?\RectorPrefix20211231\Tracy\BlueScreen $blueScreen = null)
    {
    ...
    public static function formatMessage($message) : string
    {
        if ($message instanceof \Throwable) {
            foreach (\RectorPrefix20211231\Tracy\Helpers::getExceptionChain($message) as $exception) {
                $tmp[] = ($exception instanceof \ErrorException ? \RectorPrefix20211231\Tracy\Helpers::errorTypeToString($exception->getSeverity()) . ': ' . $exception->getMessage() : \RectorPrefix20211231\Tracy\Helpers::getClass($exception) . ': ' . $exception->getMessage() . ($exception->getCode() ? ' #' . $exception->getCode() : '')) . ' in ' . $exception->getFile() . ':' . $exception->getLine();
            }
            $message = \implode("\ncaused by ", $tmp);
        } elseif (!\is_string($message)) {
            $message = \RectorPrefix20211231\Tracy\Dumper::toText($message);
        }
        return \trim($message);
    }
    /**
    ...
    {
        return \implode(' ', [\date('[Y-m-d H-i-s]'), \preg_replace('#\\s*\\r?\\n\\s*#', ' ', static::formatMessage($message)), ' @  ' . \RectorPrefix20211231\Tracy\Helpers::getSource(), $exceptionFile ? ' @@  ' . \basename($exceptionFile) : null]);
    }
    public function getExceptionFile(\Throwable $exception, string $level = self::EXCEPTION) : string
    {
        foreach (\RectorPrefix20211231\Tracy\Helpers::getExceptionChain($exception) as $exception) {
    ...
    protected function logException(\Throwable $exception, ?string $file = null) : string
    {
        ...
        $bs = $this->blueScreen ?: new \RectorPrefix20211231\Tracy\BlueScreen();
