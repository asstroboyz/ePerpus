...
class Debugger
{
    ...
    public const DEBUG = \RectorPrefix20211231\Tracy\ILogger::DEBUG, INFO = \RectorPrefix20211231\Tracy\ILogger::INFO, WARNING = \RectorPrefix20211231\Tracy\ILogger::WARNING, ERROR = \RectorPrefix20211231\Tracy\ILogger::ERROR, EXCEPTION = \RectorPrefix20211231\Tracy\ILogger::EXCEPTION, CRITICAL = \RectorPrefix20211231\Tracy\ILogger::CRITICAL;
    /********************* misc ****************d*g**/
    ...
    {
        if (!\RectorPrefix20211231\Tracy\Helpers::isCli() && self::getStrategy()->sendAssets()) {
            self::$showBar = \false;
            exit;
        }
    }
    ...
    public static function shutdownHandler() : void
    {
        ...
        if (\in_array($error['type'] ?? null, [\E_ERROR, \E_CORE_ERROR, \E_COMPILE_ERROR, \E_PARSE, \E_RECOVERABLE_ERROR, \E_USER_ERROR], \true)) {
            self::exceptionHandler(\RectorPrefix20211231\Tracy\Helpers::fixStack(new \ErrorException($error['message'], 0, $error['type'], $error['file'], $error['line'])));
        } elseif (($error['type'] ?? null) === \E_COMPILE_WARNING) {
            \error_clear_last();
            self::errorHandler($error['type'], $error['message'], $error['file'], $error['line']);
        }
        self::$reserved = null;
        if (self::$showBar && !\RectorPrefix20211231\Tracy\Helpers::isCli()) {
            try {
    ...
    public static function exceptionHandler(\Throwable $exception) : void
    {
        ...
        \RectorPrefix20211231\Tracy\Helpers::improveException($exception);
    ...
    public static function errorHandler(int $severity, string $message, string $file, int $line, ?array $context = null) : bool
    {
        ...
        if ($severity === \E_RECOVERABLE_ERROR || $severity === \E_USER_ERROR) {
            if (\RectorPrefix20211231\Tracy\Helpers::findTrace(\debug_backtrace(\DEBUG_BACKTRACE_IGNORE_ARGS), '*::__toString')) {
    ...
    /********************* services ****************d*g**/
    public static function getBlueScreen() : \RectorPrefix20211231\Tracy\BlueScreen
    {
        if (!self::$blueScreen) {
            self::$blueScreen = new \RectorPrefix20211231\Tracy\BlueScreen();
            self::$blueScreen->info = ['PHP ' . \PHP_VERSION, $_SERVER['SERVER_SOFTWARE'] ?? null, 'Tracy ' . self::VERSION];
        }
        return self::$blueScreen;
    }
    public static function getBar() : \RectorPrefix20211231\Tracy\Bar
    {
        if (!self::$bar) {
            self::$bar = new \RectorPrefix20211231\Tracy\Bar();
            self::$bar->addPanel($info = new \RectorPrefix20211231\Tracy\DefaultBarPanel('info'), 'Tracy:info');
            $info->cpuUsage = self::$cpuUsage;
            self::$bar->addPanel(new \RectorPrefix20211231\Tracy\DefaultBarPanel('errors'), 'Tracy:errors');
            // filled by errorHandler()
        }
        return self::$bar;
    }
    public static function setLogger(\RectorPrefix20211231\Tracy\ILogger $logger) : void
    {
        self::$logger = $logger;
    }
    public static function getLogger() : \RectorPrefix20211231\Tracy\ILogger
    {
        if (!self::$logger) {
            self::$logger = new \RectorPrefix20211231\Tracy\Logger(self::$logDirectory, self::$email, self::getBlueScreen());
    ...
    public static function getFireLogger() : \RectorPrefix20211231\Tracy\ILogger
    {
        if (!self::$fireLogger) {
            self::$fireLogger = new \RectorPrefix20211231\Tracy\FireLogger();
        }
    ...
    public static function getStrategy()
    {
        if (empty(self::$strategy[self::$productionMode])) {
            self::$strategy[self::$productionMode] = self::$productionMode ? new \RectorPrefix20211231\Tracy\ProductionStrategy() : new \RectorPrefix20211231\Tracy\DevelopmentStrategy(self::getBar(), self::getBlueScreen(), new \RectorPrefix20211231\Tracy\DeferredContent(self::getSessionStorage()));
        }
        return self::$strategy[self::$productionMode];
    }
    public static function setSessionStorage(\RectorPrefix20211231\Tracy\SessionStorage $storage) : void
    {
    ...
    /** @internal */
    public static function getSessionStorage() : \RectorPrefix20211231\Tracy\SessionStorage
    {
        if (!self::$sessionStorage) {
            self::$sessionStorage = \is_dir($dir = \session_save_path()) || \is_dir($dir = \ini_get('upload_tmp_dir')) || \is_dir($dir = \sys_get_temp_dir()) || ($dir = self::$logDirectory) ? new \RectorPrefix20211231\Tracy\FileSession($dir) : new \RectorPrefix20211231\Tracy\NativeSession();
        }
    ...
    {
        if ($return) {
            $options = [\RectorPrefix20211231\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20211231\Tracy\Dumper::TRUNCATE => self::$maxLength];
            return \RectorPrefix20211231\Tracy\Helpers::isCli() ? \RectorPrefix20211231\Tracy\Dumper::toText($var) : \RectorPrefix20211231\Tracy\Helpers::capture(function () use($var, $options) {
                \RectorPrefix20211231\Tracy\Dumper::dump($var, $options);
            });
        } elseif (!self::$productionMode) {
            \RectorPrefix20211231\Tracy\Dumper::dump($var, [\RectorPrefix20211231\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20211231\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20211231\Tracy\Dumper::LOCATION => self::$showLocation, \RectorPrefix20211231\Tracy\Dumper::THEME => self::$dumpTheme, \RectorPrefix20211231\Tracy\Dumper::KEYS_TO_HIDE => self::$keysToHide]);
        }
        return $var;
    }
    /**
    ...
    public static function barDump($var, ?string $title = null, array $options = [])
    {
        if (!self::$productionMode) {
            static $panel;
            if (!$panel) {
                self::getBar()->addPanel($panel = new \RectorPrefix20211231\Tracy\DefaultBarPanel('dumps'), 'Tracy:dumps');
            }
            $panel->data[] = ['title' => $title, 'dump' => \RectorPrefix20211231\Tracy\Dumper::toHtml($var, $options + [\RectorPrefix20211231\Tracy\Dumper::DEPTH => self::$maxDepth, \RectorPrefix20211231\Tracy\Dumper::TRUNCATE => self::$maxLength, \RectorPrefix20211231\Tracy\Dumper::LOCATION => self::$showLocation ?: \RectorPrefix20211231\Tracy\Dumper::LOCATION_CLASS | \RectorPrefix20211231\Tracy\Dumper::LOCATION_SOURCE, \RectorPrefix20211231\Tracy\Dumper::LAZY => \true])];
        }
        return $var;
    }
    /**
     ...
     * @return mixed
     */
    public static function log($message, string $level = \RectorPrefix20211231\Tracy\ILogger::INFO)
    {
