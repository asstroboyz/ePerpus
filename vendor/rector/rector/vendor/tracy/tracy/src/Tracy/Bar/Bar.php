...
class Bar
{
     ...
     * @return static
     */
    public function addPanel(\RectorPrefix20211231\Tracy\IBarPanel $panel, ?string $id = null) : self
    {
        if ($id === null) {
            $c = 0;
            do {
     ...
     */
    public function getPanel(string $id) : ?\RectorPrefix20211231\Tracy\IBarPanel
    {
     ...
     * @internal
     */
    public function renderLoader(\RectorPrefix20211231\Tracy\DeferredContent $defer) : void
    {
     ...
     * Renders debug bar.
     */
    public function render(\RectorPrefix20211231\Tracy\DeferredContent $defer) : void
    {
        ...
        if (\RectorPrefix20211231\Tracy\Helpers::isAjax()) {
        ...
        } elseif (\RectorPrefix20211231\Tracy\Helpers::isRedirect()) {
            if ($defer->isAvailable()) {
            ...
            if ($this->loaderRendered) {
                ...
                $nonce = \RectorPrefix20211231\Tracy\Helpers::getNonce();
                $async = \false;
                \RectorPrefix20211231\Tracy\Debugger::removeOutputBuffers(\false);
                require __DIR__ . '/assets/loader.phtml';
            }
        }
    }
    private function renderPartial(string $type, string $suffix = '') : array
    {
        ...
        return ['bar' => \RectorPrefix20211231\Tracy\Helpers::capture(function () use($type, $panels) {
            require __DIR__ . '/assets/bar.phtml';
        }), 'panels' => \RectorPrefix20211231\Tracy\Helpers::capture(function () use($type, $panels) {
    ...
    private function renderPanels(string $suffix = '') : array
    {
        ...
        foreach ($this->panels as $id => $panel) {
            ...
            try {
                ...
                $idHtml = "error-{$idHtml}";
                $tab = "Error in {$id}";
                $panelHtml = "<h1>Error: {$id}</h1><div class='tracy-inner'>" . \nl2br(\RectorPrefix20211231\Tracy\Helpers::escapeHtml($e)) . '</div>';
                unset($e);
            }
