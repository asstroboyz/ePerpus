...
class MailSender
{
    ...
    private $fromEmail;
    public function __construct(\RectorPrefix20211231\Nette\Mail\IMailer $mailer, ?string $fromEmail = null)
    {
        $this->mailer = $mailer;
    ...
    public function send($message, string $email) : void
    {
        ...
        $mail = new \RectorPrefix20211231\Nette\Mail\Message();
        $mail->setHeader('X-Mailer', 'Tracy');
        if ($this->fromEmail || \RectorPrefix20211231\Nette\Utils\Validators::isEmail("noreply@{$host}")) {
        ...
        $mail->setBody(\RectorPrefix20211231\Tracy\Logger::formatMessage($message) . "\n\nsource: " . \RectorPrefix20211231\Tracy\Helpers::getSource());
