...
final class BinaryDumper
{
     ...
     * @return string
     */
    public function toBinary(\RectorPrefix20211231\React\Dns\Model\Message $message)
    {
        $data = '';
     ...
     * @return string
     */
    private function headerToBinary(\RectorPrefix20211231\React\Dns\Model\Message $message)
    {
        $data = '';
    ...
    private function recordsToBinary(array $records)
    {
        ...
        foreach ($records as $record) {
            /* @var $record Record */
            switch ($record->type) {
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_A:
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_AAAA:
                    $binary = \inet_pton($record->data);
                    break;
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_CNAME:
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_NS:
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_PTR:
                    $binary = $this->domainNameToBinary($record->data);
                    break;
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_TXT:
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_SPF:
                    $binary = $this->textsToBinary($record->data);
                    break;
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_MX:
                    $binary = \pack('n', $record->data['priority']);
                    $binary .= $this->domainNameToBinary($record->data['target']);
                    break;
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_SRV:
                    $binary = \pack('n*', $record->data['priority'], $record->data['weight'], $record->data['port']);
                    $binary .= $this->domainNameToBinary($record->data['target']);
                    break;
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_SOA:
                    ...
                    break;
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_CAA:
                    $binary = \pack('C*', $record->data['flag'], \strlen($record->data['tag']));
                    $binary .= $record->data['tag'];
                    $binary .= $record->data['value'];
                    break;
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_SSHFP:
                    $binary = \pack('CCH*', $record->data['algorithm'], $record->data['type'], $record->data['fingerprint']);
                    break;
                case \RectorPrefix20211231\React\Dns\Model\Message::TYPE_OPT:
                    $binary = '';
                    foreach ($record->data as $opt => $value) {
                        if ($opt === \RectorPrefix20211231\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $value !== null) {
