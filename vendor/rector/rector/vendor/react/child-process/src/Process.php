...
namespace RectorPrefix20211231\React\ChildProcess;
...
use RectorPrefix20211231\React\Stream\ReadableResourceStream;
use RectorPrefix20211231\React\Stream\ReadableStreamInterface;
 ...
 */
class Process extends \RectorPrefix20211231\Evenement\EventEmitter
{
    /**
     ...
     */
    public function start(\RectorPrefix20211231\React\EventLoop\LoopInterface $loop = null, $interval = 0.1)
    {
        if ($this->isRunning()) {
            throw new \RuntimeException('Process is already running');
        }
        $loop = $loop ?: \RectorPrefix20211231\React\EventLoop\Loop::get();
        $cmd = $this->cmd;
        $fdSpec = $this->fds;
        $sigchild = null;
        ...
        foreach ($pipes as $n => $fd) {
            ...
            if ($mode === 'r+') {
                $stream = new \RectorPrefix20211231\React\Stream\DuplexResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            } elseif ($mode === 'w') {
                $stream = new \RectorPrefix20211231\React\Stream\WritableResourceStream($fd, $loop);
            } else {
                $stream = new \RectorPrefix20211231\React\Stream\ReadableResourceStream($fd, $loop);
                $stream->on('close', $streamCloseHandler);
                $closeCount++;
            }
            $this->pipes[$n] = $stream;
