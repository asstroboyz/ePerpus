...
final class DnsConnector implements \RectorPrefix20211231\React\Socket\ConnectorInterface
{
    private $connector;
    private $resolver;
    public function __construct(\RectorPrefix20211231\React\Socket\ConnectorInterface $connector, \RectorPrefix20211231\React\Dns\Resolver\ResolverInterface $resolver)
    {
    ...
    public function connect($uri)
    {
        ...
        $resolved = null;
        return new \RectorPrefix20211231\React\Promise\Promise(function ($resolve, $reject) use(&$promise, &$resolved, $uri, $connector, $host, $parts) {
            // resolve/reject with result of DNS lookup
            $promise->then(function ($ip) use(&$promise, &$resolved, $uri, $connector, $host, $parts) {
                $resolved = $ip;
                return $promise = $connector->connect(\RectorPrefix20211231\React\Socket\Connector::uri($parts, $host, $ip))->then(null, function (\Exception $e) use($uri) {
            ...
            if ($promise instanceof \RectorPrefix20211231\React\Promise\CancellablePromiseInterface) {
