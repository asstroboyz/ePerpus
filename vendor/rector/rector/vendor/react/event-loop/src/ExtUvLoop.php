 ...
 */
final class ExtUvLoop implements \RectorPrefix20211231\React\EventLoop\LoopInterface
{
    private $uv;
    ...
    public function addSignal($signal, $listener)
    {
        ...
        if (!isset($this->signalEvents[$signal])) {
            ...
            $this->signalEvents[$signal] = \RectorPrefix20211231\uv_signal_init($this->uv);
            \RectorPrefix20211231\uv_signal_start($this->signalEvents[$signal], function () use($signals, $signal) {
                $signals->call($signal);
            }, $signal);
        }
    }
    ...
    {
        if (!isset($this->streamEvents[(int) $stream])) {
            $this->streamEvents[(int) $stream] = \RectorPrefix20211231\uv_poll_init_socket($this->uv, $stream);
        }
